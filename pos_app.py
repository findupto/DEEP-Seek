...
class App(tk.Tk):
    ...
    def __init__(self,s,user):
        super().__init__(); self.s=s; self.user=user; self.cart={}; self.pm=PrinterManager(); self.title(BUSINESS["name"]+" — POS"); self.geometry("1400x850"); self.minsize(900,600); st=ttk.Style(self); st.theme_use("clam"); st.configure("TButton",padding=(9,7)); st.configure("Primary.TButton",background="#2563eb",foreground="white",font=("Segoe UI",10,"bold"),padding=(12,10)); st.configure("Title.TLabel",font=("Segoe UI",22,"bold")); st.configure("Treeview",rowheight=30,font=("Segoe UI",10)); st.configure("Treeview.Heading",font=("Segoe UI",10,"bold")); self.build_shell(); self.show("POS"); self.after(700,self.pm.auto_detect_and_connect)
