...
def install(App, Store=None):
    ...
    def init_db(self):
        ...
        CREATE TABLE IF NOT EXISTS account_transactions(
            id INTEGER PRIMARY KEY AUTOINCREMENT, party_type TEXT NOT NULL,
            party_id INTEGER NOT NULL, txn_type TEXT NOT NULL, amount REAL NOT NULL CHECK(amount>0),
            method TEXT DEFAULT 'Cash', note TEXT DEFAULT '', created_at TEXT NOT NULL, user_id INTEGER);
    ...
    def app_init(self,*args,**kwargs):
        old_init(self,*args,**kwargs); init_db(self)
    ...
    def post_account_tx(self,party_type,party_id,txn_type,amount,method="Cash",note=""):
        ...
        if party_type=="Customer":
            if txn_type=="Payment":
                self.s.q("UPDATE customers SET balance=MAX(0,COALESCE(balance,0)-?) WHERE id=?",(amount,party_id)); lines=[("1000" if method=="Cash" else "1010",amount,0,"Customer payment"),("1100",0,amount,"Customer receivable settled")]
            elif txn_type=="Charge":
                self.s.q("UPDATE customers SET balance=COALESCE(balance,0)+? WHERE id=?",(amount,party_id)); lines=[("1100",amount,0,"Customer receivable"),("4000",0,amount,"Customer charge")]
            elif txn_type=="Refund":
                self.s.q("UPDATE customers SET balance=MAX(0,COALESCE(balance,0)-?) WHERE id=?",(amount,party_id)); lines=[("4000",amount,0,"Customer refund"),("1000" if method=="Cash" else "1010",0,amount,"Refund paid")]
            else: raise ValueError("Invalid customer transaction")
        elif party_type=="Supplier":
            if txn_type=="Payment":
                self.s.q("UPDATE suppliers SET balance=MAX(0,COALESCE(balance,0)-?) WHERE id=?",(amount,party_id)); lines=[("2000",amount,0,"Supplier payable settled"),("1000" if method=="Cash" else "1010",0,amount,"Supplier payment")]
            elif txn_type=="Charge":
                self.s.q("UPDATE suppliers SET balance=COALESCE(balance,0)+? WHERE id=?",(amount,party_id)); lines=[("1200",amount,0,"Supplier charge"),("2000",0,amount,"Supplier payable")]
    ...
    def account_dialog(self,party_type,party_id,actions):
        ...
        if not row:return
        w=self.dialog(f"{party_type} Account — {row['name']}",820,620); f=ttk.Frame(w,padding=15); f.pack(fill="both",expand=True)
    ...
    def account_tx_dialog(self,party_type,party_id,txn_type,parent):
        ...
        def save():
            try:
                self.post_account_transaction(party_type,party_id,txn_type,amount.get(),method.get(),note.get().strip()); w.destroy(); parent.destroy(); self.show("Customers" if party_type=="Customer" else "Suppliers" if party_type=="Supplier" else "Staff")
    ...
    # One controlled expense entry can optionally settle a supplier/staff or
    # post against a customer while still remaining an expense record.
    def expense_account_dialog(self):
        w=self.dialog("Expense / Account Posting",520,470); f=ttk.Frame(w,padding=18); f.pack(fill="both",expand=True)
        category=tk.StringVar(value="General Expense"); amount=tk.StringVar(); party=tk.StringVar(value="Cash / General"); method=tk.StringVar(value="Cash"); note=tk.StringVar()
        ttk.Label(f,text="Expense / Reason").pack(anchor="w"); ttk.Entry(f,textvariable=category).pack(fill="x",pady=5)
        ...
        ttk.Label(f,text="Post To Account").pack(anchor="w"); ttk.Combobox(f,textvariable=party,values=choices,state="readonly").pack(fill="x",pady=5)
        ttk.Label(f,text="Payment Method").pack(anchor="w"); ttk.Combobox(f,textvariable=method,values=["Cash","Card","Other"],state="readonly").pack(fill="x",pady=5)
        ttk.Label(f,text="Note").pack(anchor="w"); ttk.Entry(f,textvariable=note).pack(fill="x",pady=5)
        def save():
            try:
                ...
                cur=self.s.q("INSERT INTO expenses(category,amount,note,created_at,user_id,party_type,party_id,payment_method) VALUES(?,?,?,?,?,?,?,?)",(category.get().strip() or "General Expense",a,note.get().strip(),now(),self.user["id"],ptype,pid,method.get()))
                eid=cur.lastrowid
                if ptype=="Supplier":
                    # A supplier expense is money paid on their account.
                    self.s.q("UPDATE suppliers SET balance=MAX(0,COALESCE(balance,0)-?) WHERE id=?",(a,pid)); self.post_account_transaction("Supplier",pid,"Payment",a,method.get(),category.get().strip())
                elif ptype=="Staff":
                    self.post_account_transaction("Staff",pid,"Advance",a,method.get(),category.get().strip())
                else:
                    try:
    ...
    if old_expenses:
        def page_expenses(self):
            old_expenses(self)
            bar=ttk.LabelFrame(self.bodyinner,text="ACCOUNT-AWARE EXPENSE",padding=8); bar.pack(fill="x",pady=(8,0),before=self.bodyinner.winfo_children()[-1] if self.bodyinner.winfo_children() else None)
            ttk.Button(bar,text="ADD EXPENSE + POST TO ACCOUNT",style="Primary.TButton",command=self.expense_account_dialog).pack(side="left")
        ...
        if old_page:
            def make_page(old_page=old_page,party=party,actions=actions):
                def page(self):
                    ...
                    if tree is not None:
                        bar=ttk.LabelFrame(self.bodyinner,text="ACCOUNT TRANSACTIONS",padding=8); bar.pack(fill="x",pady=(8,0),before=self.bodyinner.winfo_children()[-1] if self.bodyinner.winfo_children() else None)
