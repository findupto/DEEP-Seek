"""Stable final Products/Menu runtime layer."""
import csv
import sqlite3
...
def install(App):
    if getattr(App, "_catalog_runtime_fix_v3", False):
        return App
    ...
    def init_catalog(self):
        ...
        CREATE INDEX IF NOT EXISTS idx_audit_product ON audit_log(entity,entity_id);
        """)
        self.s.c.commit()
    ...
    def selected(self):
        ...
        try:
            ...
            messagebox.showerror("Product", "Invalid product selection.", parent=self)
            return None
    ...
    def reload_products(self, all_rows=False):
        ...
        if not all_rows:
            sql += " AND active=1"
        qv = getattr(self, "prod_filter", None)
        cv = getattr(self, "cat_filter", None)
        query = qv.get().strip().lower() if qv else ""
        category = cv.get() if cv else "All"
        if query:
    ...
    def catalog_edit(self):
        init_catalog(self)
        old = selected(self) if getattr(self, "pr", None) and self.pr.selection() else None
        w = self.dialog("Edit Product" if old else "Add Product", 560, 620)
        ...
        for key, label, value in fields:
            ...
            vars_[key] = tk.StringVar(value=str(value or "")); ttk.Entry(f, textvariable=vars_[key]).pack(fill="x")
        visible = tk.BooleanVar(value=bool(old["active"]) if old else True)
        ttk.Checkbutton(f, text="Available in POS menu", variable=visible).pack(anchor="w", pady=8)

        def save():
            try:
                name = vars_["name"].get().strip(); category = vars_["category"].get().strip() or "General"
                price = float(vars_["price"].get()); cost = float(vars_["cost"].get() or 0); stock = float(vars_["stock"].get() or 0); barcode = vars_["barcode"].get().strip()
                if not name: raise ValueError("Product name is required.")
                if price < 0 or cost < 0 or stock < 0: raise ValueError("Numeric values cannot be negative.")
                if barcode:
                    dup = self.s.q("SELECT id FROM products WHERE barcode=? AND id<>? LIMIT 1", (barcode, old["id"] if old else 0)).fetchone()
                    if dup: raise ValueError("That barcode/SKU is already used by another product.")
                self.s.q("INSERT OR IGNORE INTO product_categories(name,active) VALUES(?,1)", (category,))
                active = 1 if visible.get() else 0
                if old:
                    old_stock = float(old["stock"] or 0)
                    self.s.q("UPDATE products SET name=?,category=?,price=?,cost=?,stock=?,barcode=?,active=? WHERE id=?", (name, category, price, cost, stock, barcode, active, old["id"]))
                    pid, action = old["id"], "UPDATE"
                    delta = stock - old_stock
                    if abs(delta) > 1e-9:
                        self.s.q("INSERT INTO stock_movements(product_id,qty,movement_type,note,created_at,user_id) VALUES(?,?,?,?,?,?)", (pid, delta, "Manual Stock Adjustment", "Product edit", datetime.now().isoformat(timespec="seconds"), self.user["id"]))
                else:
                    cur = self.s.q("INSERT INTO products(name,category,price,cost,stock,barcode,active) VALUES(?,?,?,?,?,?,?)", (name, category, price, cost, stock, barcode, active))
                    pid, action = cur.lastrowid, "CREATE"
                    if stock:
                        self.s.q("INSERT INTO stock_movements(product_id,qty,movement_type,note,created_at,user_id) VALUES(?,?,?,?,?,?)", (pid, stock, "Opening Stock", "Product creation", datetime.now().isoformat(timespec="seconds"), self.user["id"]))
                ...
                messagebox.showerror("Product", str(exc), parent=w)
    ...
    def product_delete(self):
        ...
        if not p: return
        if not p["active"]:
            messagebox.showinfo("Product", "This product is already archived.", parent=self); return
        if not messagebox.askyesno("Archive Product", f"Remove '{p['name']}' from the active POS menu?\n\nSales, purchases and history will be preserved.", parent=self): return
    ...
    def product_restore(self):
        p = selected(self)
        if not p: return
        if p["active"]:
            messagebox.showinfo("Product", "This product is already active.", parent=self); return
        self.s.q("UPDATE products SET active=1 WHERE id=?", (p["id"],)); self.s.c.commit(); self.s.audit(self.user, "RESTORE", "product", p["id"], p["name"]); reload_products(self, True)

    def product_delete_all(self):
        count = int(self.s.q("SELECT COUNT(*) n FROM products WHERE active=1").fetchone()["n"] or 0)
        if not count: return messagebox.showinfo("Menu", "There are no active menu products.", parent=self)
        if not messagebox.askyesno("DELETE ALL MENU", f"Archive ALL {count} active menu products?\n\nOld orders, sales and product history will remain intact.", parent=self): return
        ...
        messagebox.showinfo("Menu", f"{count} products were removed from the active POS menu.", parent=self)
            ...
            with open(path, "r", encoding="utf-8-sig", newline="") as fh:
                ...
                for line_no, raw in enumerate(reader, start=2):
                    try:
                        ...
                        self.s.q("INSERT OR IGNORE INTO product_categories(name,active) VALUES(?,1)", (category,))
                        existing = None
                        if barcode: existing = self.s.q("SELECT id FROM products WHERE barcode=? LIMIT 1", (barcode,)).fetchone()
                        if not existing: existing = self.s.q("SELECT id FROM products WHERE lower(name)=lower(?) LIMIT 1", (name,)).fetchone()
                        if existing:
                            pid = existing["id"]; old_stock = float(self.s.q("SELECT stock FROM products WHERE id=?", (pid,)).fetchone()["stock"] or 0)
                            self.s.q("UPDATE products SET name=?,category=?,price=?,cost=?,stock=?,barcode=?,active=? WHERE id=?", (name, category, price, cost, stock, barcode, active, pid)); delta = stock - old_stock
                            if abs(delta) > 1e-9: self.s.q("INSERT INTO stock_movements(product_id,qty,movement_type,note,created_at,user_id) VALUES(?,?,?,?,?,?)", (pid, delta, "Bulk Stock Adjustment", "Bulk CSV import", datetime.now().isoformat(timespec="seconds"), self.user["id"]))
                            updated += 1
                        else:
                            ...
                            if stock: self.s.q("INSERT INTO stock_movements(product_id,qty,movement_type,note,created_at,user_id) VALUES(?,?,?,?,?,?)", (pid, stock, "Opening Stock", "Bulk CSV import", datetime.now().isoformat(timespec="seconds"), self.user["id"]))
                    except Exception as exc:
                        skipped += 1; errors.append(f"Line {line_no}: {exc}")
            self.s.c.commit(); self.s.audit(self.user, "BULK_IMPORT", "product", None, f"created={created}, updated={updated}, skipped={skipped}"); reload_products(self, True)
            msg = f"Import complete.\n\nCreated: {created}\nUpdated: {updated}\nSkipped: {skipped}" + (("\n\nFirst errors:\n" + "\n".join(errors[:8])) if errors else "")
            messagebox.showinfo("Menu Import", msg, parent=self)
        except Exception as exc:
            try: self.s.c.rollback()
            except Exception: pass
            messagebox.showerror("Menu Import", str(exc), parent=self)
    ...
    def bulk_center(self):
        w = self.dialog("Bulk Menu Operations", 580, 430); f = ttk.Frame(w, padding=18); f.pack(fill="both", expand=True)
        ttk.Label(f, text="Bulk Menu Operations", font=("Segoe UI", 18, "bold")).pack(anchor="w")
        ttk.Label(f, text="Upload a CSV to create/update products, or download the complete menu. Existing history is preserved.", foreground="#64748b", wraplength=520).pack(anchor="w", pady=(5, 16))
        ttk.Button(f, text="UPLOAD / IMPORT MENU CSV", style="Primary.TButton", command=lambda: product_import(self)).pack(fill="x", pady=5)
        ttk.Button(f, text="DOWNLOAD / EXPORT MENU CSV", command=lambda: product_export(self)).pack(fill="x", pady=5)
        ttk.Button(f, text="DOWNLOAD CSV TEMPLATE", command=lambda: product_template(self)).pack(fill="x", pady=5)
        ttk.Button(f, text="CLOSE", command=w.destroy).pack(fill="x", pady=(18, 0))
    ...
    def catalog_history(self):
        ...
        ttk.Label(f, text=f"{p['name']}  |  Category: {p['category']}  |  Stock: {p['stock']}  |  Sale: Rs. {float(p['price']):,.2f}", font=("Segoe UI", 14, "bold")).pack(anchor="w", pady=5)
        ttk.Label(f, text="Sales, stock movements, purchases and audit events.", foreground="#64748b").pack(anchor="w", pady=(0, 8))
        t = self.table(f, ("date", "type", "qty", "reference", "user"), {"date": "Date", "type": "Event", "qty": "Qty / Value", "reference": "Reference", "user": "User"}, 18)
        rows = self.s.rows("""
            SELECT s.created_at date,'SALE' type,si.quantity qty,s.invoice_no reference,COALESCE(u.username,'') user
            FROM sale_items si JOIN sales s ON s.id=si.sale_id LEFT JOIN users u ON u.id=s.user_id WHERE si.product_id=?
            UNION ALL SELECT m.created_at,m.movement_type,m.qty,COALESCE(m.note,''),COALESCE(u.username,'') FROM stock_movements m LEFT JOIN users u ON u.id=m.user_id WHERE m.product_id=?
            UNION ALL SELECT p.created_at,'PURCHASE',pi.quantity,p.invoice_no,COALESCE(u.username,'') FROM purchase_items pi JOIN purchases p ON p.id=pi.purchase_id LEFT JOIN users u ON u.id=p.user_id WHERE pi.product_id=?
            UNION ALL SELECT a.created_at,a.action,'',COALESCE(a.details,''),COALESCE(u.username,'') FROM audit_log a LEFT JOIN users u ON u.id=a.user_id WHERE a.entity='product' AND a.entity_id=?
            ORDER BY date DESC
        """, (p["id"], p["id"], p["id"], p["id"]))
    ...
    def categories(self):
        ...
        def add_():
            ...
            name = name.strip()
            try:
                self.s.q("INSERT INTO product_categories(name,active) VALUES(?,1)", (name,)); self.s.c.commit(); self.s.audit(self.user, "CREATE", "category", None, name); reload_()
            except sqlite3.IntegrityError:
                row = self.s.q("SELECT id FROM product_categories WHERE name=?", (name,)).fetchone()
                if row:
                    self.s.q("UPDATE product_categories SET active=1 WHERE id=?", (row["id"],)); self.s.c.commit(); reload_()
                else: messagebox.showerror("Category", "Category already exists.", parent=w)
        def archive_():
            sel = t.selection()
            if not sel: return
            self.s.q("UPDATE product_categories SET active=0 WHERE id=?", (int(sel[0]),)); self.s.c.commit(); reload_()
        def restore_():
            sel = t.selection()
            if sel: self.s.q("UPDATE product_categories SET active=1 WHERE id=?", (int(sel[0]),)); self.s.c.commit(); reload_()
        b = ttk.Frame(f); b.pack(fill="x", pady=8); ttk.Button(b, text="ADD CATEGORY", style="Primary.TButton", command=add_).pack(side="left"); ttk.Button(b, text="ARCHIVE", command=archive_).pack(side="left", padx=5); ttk.Button(b, text="RESTORE", command=restore_).pack(side="left", padx=5); reload_()
    ...
    def modifiers(self):
        ...
        for r in self.s.rows("SELECT i.id,m.name modifier,i.name item,i.price_delta FROM product_modifier_items i JOIN product_modifiers m ON m.id=i.modifier_id WHERE i.active=1 AND m.active=1 ORDER BY m.name,i.name"):
            t.insert("", "end", iid=str(r["id"]), values=(r["id"], r["modifier"], r["item"], f"{float(r['price_delta']):+.2f}"))
        def add_():
            d = self.dialog("Add Modifier Option", 430, 350); q = ttk.Frame(d, padding=16); q.pack(fill="both", expand=True); group, item, delta = tk.StringVar(), tk.StringVar(), tk.StringVar(value="0")
            ...
            def save():
                ...
                try:
                    amount = float(delta.get() or 0); self.s.q("INSERT OR IGNORE INTO product_modifiers(name,active) VALUES(?,1)", (group.get().strip(),)); mid = self.s.q("SELECT id FROM product_modifiers WHERE name=?", (group.get().strip(),)).fetchone()["id"]; self.s.q("INSERT INTO product_modifier_items(modifier_id,name,price_delta,active) VALUES(?,?,?,1)", (mid, item.get().strip(), amount)); self.s.c.commit(); d.destroy(); w.destroy(); self.modifiers()
                except Exception as exc:
                    try: self.s.c.rollback()
                    except Exception: pass
                    messagebox.showerror("Modifier", str(exc), parent=d)
            ttk.Button(q, text="SAVE", style="Primary.TButton", command=save).pack(fill="x", pady=15)
        def archive_():
            sel = t.selection()
            if sel: self.s.q("UPDATE product_modifier_items SET active=0 WHERE id=?", (int(sel[0]),)); self.s.c.commit(); w.destroy(); self.modifiers()
        b = ttk.Frame(f); b.pack(fill="x", pady=8); ttk.Button(b, text="ADD MODIFIER OPTION", style="Primary.TButton", command=add_).pack(side="left"); ttk.Button(b, text="REMOVE OPTION", command=archive_).pack(side="left", padx=5)

    def page(self):
        init_catalog(self); self.title("Products & Menu", "Manage products, media, history, categories and bulk menu files."); root = getattr(self, "bodyinner", self.body)
        bar = ttk.Frame(root); bar.pack(fill="x", pady=8)
        actions = [("ADD PRODUCT", lambda: catalog_edit(self), True), ("EDIT SELECTED", lambda: catalog_edit(self), False), ("VIEW HISTORY", lambda: catalog_history(self), False), ("DELETE / ARCHIVE", lambda: product_delete(self), False), ("RESTORE", lambda: product_restore(self), False), ("DELETE ALL MENU", lambda: product_delete_all(self), False), ("IMAGE / ICON / EMOJI / GIFT", lambda: getattr(self, "product_media", lambda: messagebox.showinfo("Product Visuals", "Product visuals are not available.", parent=self))(), False)]
        for text, command, primary in actions: ttk.Button(bar, text=text, style="Primary.TButton" if primary else "TButton", command=command).pack(side="left", padx=(0, 4))
    ...
    App.init_catalog = init_catalog
    App._selected_product = selected
    App.reload_products = reload_products
    App.load_products = lambda self: reload_products(self, False)
    App.load_products_all = lambda self: reload_products(self, True)
    App.catalog_edit = catalog_edit
    App.product_delete = product_delete
    App.product_restore = product_restore
    ...
    App._catalog_runtime_fix_v3 = True
    return App
