...
class PrinterManager:
    ...
    def disconnect(self):
        if self.device and self.device.get("transport")=="BLE-GATT" and self.sock is not None and self._ble:
            try:self._ble.run(self.sock.disconnect(),timeout=5)
    ...
    async def _ble_connect_async(self,address,preferred=None):
        ...
        if not writable:
            await client.disconnect(); raise RuntimeError("Bluetooth LE connected, but no writable GATT characteristic was exposed. This device may not be an ESC/POS printer.")
        chosen=writable[0]; preferred=(preferred or "").lower()
        for ch in writable:
            if str(ch.uuid).lower()==preferred: chosen=ch; break
    ...
    def auto_reconnect(self):
        ...
        try:
            ...
            for r in rows:
                if saved.get("type")=="Bluetooth LE" and r.get("type")=="Bluetooth LE" and str(r.get("address","")).lower()==str(saved.get("address","")).lower(): candidates.append(r)
                elif str(saved.get("type","")) in ("Bluetooth/Serial COM","Serial/COM") and r.get("type") in ("Bluetooth/Serial COM","Serial/COM") and self._com_matches_saved(r,saved): candidates.append(r)
                elif saved.get("type")=="Windows Printer" and r.get("type")=="Windows Printer" and str(r.get("name","")).lower()==str(saved.get("name","")).lower(): candidates.append(r)
            if not candidates and saved.get("type") in ("Bluetooth/Serial COM","Serial/COM"): candidates=[r for r in rows if r.get("type") in ("Bluetooth/Serial COM","Serial/COM")]
            for r in candidates:
                try:
    ...
    def auto_detect_and_connect(self,callback=None):
        def worker():
            ...
            if callback: callback("Reconnected to saved printer" if ok else "Saved printer could not be reconnected.")
        threading.Thread(target=worker,daemon=True).start()
    ...
    def write_raw(self,data):
        ...
        if self.device and self.device.get("transport")=="BLE-GATT":
            ...
            self._ble.run(send(),timeout=15); return
...
class PrinterSettings(tk.Toplevel):
    ...
    def reconnect(self):
        ...
        def worker():
            ok=self.m.auto_reconnect(); err=self.m.status().get("error",""); self.after(0,lambda:self._reconnect_done(ok,err))
        threading.Thread(target=worker,daemon=True).start()
