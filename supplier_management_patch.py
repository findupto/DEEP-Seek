...
def install(App):
    ...
    def page_suppliers(self):
        ...
        ttk.Button(
            ...
            command=lambda: self.supplier_edit(),
        ).pack(side="left")
        ttk.Button(
            bar,
            text="EDIT SELECTED",
            command=lambda: self.supplier_edit(True),
        ).pack(side="left", padx=4)
        ttk.Button(
            bar,
            text="VIEW HISTORY",
            command=self.supplier_history,
        ).pack(side="left", padx=4)
        ttk.Button(
            bar,
            text="REMOVE / ARCHIVE",
            command=self.supplier_archive,
        ).pack(side="left", padx=4)
        ttk.Button(
            bar,
            text="RESTORE ARCHIVED",
            command=self.supplier_restore,
        ).pack(side="left", padx=4)
        ttk.Button(
            bar,
            text="REFRESH",
            command=self.page_suppliers,
        ).pack(side="left", padx=4)
        ...
        ttk.Checkbutton(
            filterbar,
            text="Show archived",
            variable=self.supplier_show_archived,
            command=self._supplier_reload,
        ).pack(side="left")
        ...
        self.supplier_tree.bind(
            "<Double-1>", lambda _e: self.supplier_history()
        )
    ...
    def _supplier_reload(self):
        ...
        args = []
        sql += " AND active=?"
        args.append(0 if archived else 1)

        if search:
            sql += (
                " AND lower(COALESCE(name,'') || ' ' || "
                "COALESCE(phone,'') || ' ' || COALESCE(address,'')) LIKE ?"
            )
            args.append("%" + search + "%")
        ...
        for r in self.s.rows(sql, tuple(args)):
            tree.insert(
                ...
                values=(
                    ...
                    "Archived" if not r["active"] else "Active",
                ),
            )
    ...
    def supplier_edit(self, editing=False):
        ...
        w = self.dialog(
            "Edit Supplier" if old else "Add Supplier",
            520,
            440,
        )
        ...
        vars_ = {}
        fields = [
            ("name", "Supplier Name", old["name"] if old else ""),
            ("phone", "Phone", old["phone"] if old else ""),
            ("address", "Address", old["address"] if old else ""),
        ]
        ...
        if old:
            ttk.Label(
                f,
                text=f"Current balance: {self.money(old['balance'])}",
                foreground="#64748b",
        ...
        def save():
            ...
            if old:
                ...
                cur = self.s.q(
                    "INSERT INTO suppliers(name,phone,address,active) VALUES(?,?,?,1)",
                    (
                        name,
                        vars_["phone"].get().strip(),
                        vars_["address"].get().strip(),
    ...
    def supplier_archive(self):
        ...
        if not messagebox.askyesno(
            "Remove Supplier",
            f"Remove '{p['name']}' from the active supplier list?\n\n"
            "Purchase and payment history will be preserved.",
            parent=self,
        ):
            return
    ...
    def supplier_restore(self):
        ...
        self.s.q(
            "UPDATE suppliers SET active=1 WHERE id=?",
            (p["id"],),
        )
        self.s.c.commit()
        self.s.audit(
    ...
    def supplier_history(self):
        ...
        if not p:
            return
        # Reuse the existing real supplier transaction/history implementation.
